"""Deploy AURA Sales IQ to Vertex AI Agent Engine."""
import json
import logging
import os
import shutil
...
_config_path = _PROJECT_ROOT / "AURA_orch" / "tools_config.json"
...
logger.info("Deploying AURA Sales IQ agent to %s / %s …", GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_LOCATION)
    ...
    if existing_id:
        try:
            ...
            remote_app = agent_engines.create(
                wrapped,
                display_name="AURA Sales IQ — Revenue Accelerator (Laabu)",
                requirements=REQUIREMENTS,
                extra_packages=["./stratova_shared", "./tools"],
                env_vars=agent_env_vars,
            )
...
try:
    ...
    _sid = "laabu-agents-sales-iq-engine-id"
