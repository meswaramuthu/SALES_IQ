...
Usage:
    ...
    uv run python scripts/upload_config.py config/tools_config.json --gcs-uri gs://bucket/path/config.json
"""
import argparse
import json
import os
...
def main() -> None:
    ...
    enabled = [k for k, v in config.get("tools", {}).items() if v.get("enabled")]
    print(f"Uploaded to {args.gcs_uri}")
    print(f"Enabled tools: {', '.join(enabled) or '(none)'}")
