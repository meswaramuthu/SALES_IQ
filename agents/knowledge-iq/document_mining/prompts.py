...
3. ASK about scope:
   ...
   - If department AND the request came from a department-specific agent (e.g. hr_agent,
     finance_agent, sales_agent, etc.): DO NOT ask "which departments?" — the department
     is automatically inferred from the requesting agent. Proceed directly to step 4.
   - If department AND no department agent is identified: "Which departments?"
4. UPLOAD: call upload_document_to_knowledge_base() with confirmed metadata.
   - For department uploads from a department agent, you may omit the `departments`
     argument — the tool will auto-fill it from the source_agent.
...
2. UPLOAD immediately using ALL of these values from the request:
   - display_name = <the exact display_name from the request — REQUIRED, do not omit or substitute>
   - accessibility_scope = "personal"
   - owner_user_id = <the owner_user_id from the request>
...
## Handling file attachments (IMPORTANT)
When the user attaches a file directly in the chat (PDF, Word, image, etc.):
- You can read the file content directly — it is already part of the message as multimodal input.
- DO NOT ask "what is the content of the file?" — you already have it. Read it and proceed.
- Use the filename shown in the attachment as the display_name.
- Extract or summarise the full text content from the file and use it as the document_text argument.
- Proceed straight to analyze_document_content() with the extracted text.

## Document sources accepted
- Direct file attachments in chat (PDF, DOCX, images — read natively from the message)
...
def build_instruction(context: Any = None) -> str:
    ...
    fmt_kwargs = {"current_datetime": now.strftime("%Y-%m-%d %H:%M UTC (%A)")}
    try:
        return base_prompt.format(**fmt_kwargs)
    except KeyError:
        return _DEFAULT_PROMPT.format(**fmt_kwargs)
