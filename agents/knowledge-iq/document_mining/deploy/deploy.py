"""Deploy document-mining agent to Vertex AI Agent Engine (always creates a fresh instance).

Run from anywhere inside the enterpriseGPT uv environment so all deps are available:
    cd agents/knowledge-iq/enterpriseGPT
    uv run python ../document_mining/deploy/deploy.py
"""
...
# If another agent was imported earlier in the same Python process (e.g. when
# running multiple deploy scripts back-to-back), the generic names "agent",
# "config", and "prompts" may be cached pointing to the wrong code.  Evict
# them so the explicit file-path loads below always get a fresh module.
...
# ── Load agent modules from explicit file paths (not by name) ────────────────
# This guarantees we always load document_mining's own files regardless of
# what is on sys.path or cached in sys.modules.
...
# tools/ must be on sys.path before agent modules are loaded (they import from it)
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

vertexai.init(project=PROJECT, location=LOCATION, staging_bucket=BUCKET)

log.info("Loading document-mining modules from explicit file paths …")
...
# Register the three local modules by value so cloudpickle embeds their source
...
# Config values (corpus, registry URI) are hardcoded in the GCS tools_config.json
# so we only need the GCS config pointer here — no RAG_CORPUS / SCOPE_REGISTRY_URI.
...
log.info("Deploying document-mining agent → %s / %s", PROJECT, LOCATION)
...
with tempfile.TemporaryDirectory(prefix="dm_bundle_") as _tmp:
    bundle = Path(_tmp)
...
try:
    ...
    _sid = "laabu-agents-knowledge-iq-dm-engine-id"
...
try:
    ...
    # List existing agents and find the old DM entry
    _req = urllib.request.Request(_agents_url, headers=_headers)
    with urllib.request.urlopen(_req, timeout=15) as _r:
        _existing_agents = _json.loads(_r.read()).get("agents", [])

    for _a in _existing_agents:
        _rn = (_a.get("adkAgentDefinition", {})
                  .get("provisionedReasoningEngine", {})
                  .get("reasoningEngine", ""))
        # Match by known old DM engine IDs
        _engine_id = _rn.split("/")[-1]
        if _engine_id in ("8073633068654526464", "12389910483515381170"):
            ...
            try:
                ...
                log.info("Removed old Agentspace entry: %s (engine: %s)", _a["name"], _engine_id)
            except Exception as _de:
                log.warning("Could not delete old entry %s: %s", _a["name"], _de)

    # Register new entry
    _body = _json.dumps({
        "displayName": "Laabu Document Intelligence",
        "description": (
            "Intelligent document ingestion for the organisational knowledge base. "
            "Analyses content, suggests metadata tags, confirms accessibility scope "
            "(organisation-wide, department-restricted, or personal-private), and "
            "ingests documents into the RAG corpus with full provenance tracking."
        ),
        "adkAgentDefinition": {
...
# ── Update orchestrator + personal_assistant GCS configs ─────────────────────
log.info("Patching dependent agent GCS configs with new DM resource name …")
try:
    import json as _json
    ...
    def _gcs_read(uri: str) -> dict:
        ...
        blob = _gcs_client.bucket(_b).blob(_p)
    ...
    _DEPENDENT_CFGS = [
        ...
        "gs://stratova-platform/agents/knowledge-iq/personal-copilot/tools_config.json",
    ]
    _OLD_DM_IDS = {"8073633068654526464", "12389910483515381170"}

    for _cfg_uri in _DEPENDENT_CFGS:
        try:
            _cfg = _gcs_read(_cfg_uri)
            _changed = False
            for _section in (_cfg.get("sub_agents", {}).values(),
                              [_cfg.get("document_mining_agent", {}),
                               _cfg.get("tools", {}).get("document_mining", {})]):
                for _item in _section:
                    if not isinstance(_item, dict):
                        continue
                    _rn = _item.get("resource_name", "")
                    if _rn.split("/")[-1] in _OLD_DM_IDS:
                        _item["resource_name"] = resource_name
                        _changed = True
            if _changed:
                _gcs_write(_cfg_uri, _cfg)
                log.info("Updated DM resource name in: %s", _cfg_uri)
            else:
                log.info("No old DM resource name found in: %s (may already be current)", _cfg_uri)
        except Exception as _cu:
            log.warning("Could not update %s: %s", _cfg_uri, _cu)
except Exception as _dep_exc:
    log.warning("Dependent agent config update failed (non-fatal): %s", _dep_exc)

print(f"\n{'='*65}")
print(f"  Document-Mining Agent deployed!")
