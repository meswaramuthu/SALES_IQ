...
log.info("Importing document-mining agent …")
from agent import root_agent  # noqa: E402

# Register local modules by value so cloudpickle embeds their code in the pickle
...
for key in ("RAG_CORPUS", "SCOPE_REGISTRY_URI"):
    val = os.getenv(key, "")
    if val:
        agent_env_vars[key] = val

# Provide GCS config URI if the agent should load config from GCS
...
with tempfile.TemporaryDirectory(prefix="dm_bundle_") as _tmp:
    ...
    remote_app = agent_engines.create(
        wrapped,
        display_name="Knowledge-IQ — Document Mining (Laabu)",
