"""Update the existing personal-assistant Agent Engine in-place (preserves resource ID).

Run from the enterpriseGPT uv environment:
    cd agents/knowledge-iq/enterpriseGPT
    uv run python ../personal_assistant/deploy/update_engine.py
"""
...
log.info("Loading personal_assistant modules …")
...
with tempfile.TemporaryDirectory(prefix="pa_update_") as _tmp:
    bundle = Path(_tmp)
...
print(f"\n{'='*65}")
print(f"  Personal Assistant Agent updated in-place!")
