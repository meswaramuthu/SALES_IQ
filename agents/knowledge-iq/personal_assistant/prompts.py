...
- You CAN read the file content directly — it is already in your multimodal context.
- DO NOT ask the user "what is the content of the file?" or "please provide the text" — you already have it.
- Read every page of the file and extract ALL visible text (headings, paragraphs, tables, bullets — everything).
- Use the filename shown in the attachment as the display_name.
- Call upload_to_personal_knowledge(document_text=<extracted text>, display_name=<filename>) immediately.
...
- "upload this / save this for me" OR any file attachment → upload_to_personal_knowledge
...
def build_instruction(context: Any = None) -> str:
    ...
    try:
        ...
        # GCS prompt may contain extra placeholders (e.g. {available_sub_agents});
        # fall back to the default prompt which only uses {current_datetime}.
