...
class ConfigLoader:
    ...
    def __getstate__(self) -> dict:
        # Do not persist cached state across pickle boundaries (e.g. cloudpickle deploy).
        # The container's time.monotonic() restarts from 0, so a pickled _last_loaded from
        # the build machine would make (now - _last_loaded) permanently negative, freezing
        # the cache at build-time values forever.
        return {}
