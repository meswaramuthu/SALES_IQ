...
def _dept_from_agent(source_agent: str, agent_dept_map: dict) -> str:
    """Return the department for a source agent using the config map, or '' if unknown."""
    if not source_agent:
        return ""
    name = source_agent.lower().strip()
    # Config map takes priority
    if name in agent_dept_map:
        return agent_dept_map[name]
    # Fallback: bare dept name or "<dept>_agent" pattern
    for dept in AVAILABLE_DEPARTMENTS:
        if name == dept or name == f"{dept}_agent":
            return dept
    return ""

_AGENTSPACE_START_RE = re.compile(r"<start_of_user_uploaded_file:\s*(.+?)>")


def _detect_agentspace_filename(tool_context) -> str:
    """Return the filename from an Agentspace file-attachment placeholder, or ''."""
    try:
        ...
        for part in parts:
            ...
            m = _AGENTSPACE_START_RE.search(txt)
            if m:
...
def build_rag_ingest_tools(config_getter: Callable) -> list[Callable]:
    ...
    def analyze_document_content(
        ...
        if not content_sample or not content_sample.strip():
            # Check for Agentspace file placeholder — prompt LLM to extract and retry
            attached_name = _detect_agentspace_filename(tool_context)
            if attached_name:
                return {
                    "status": "action_required",
                    "filename": attached_name,
                    "message": (
                        f"The file '{attached_name}' is in your multimodal context — you CAN read it.\n\n"
                        "ACTION REQUIRED: Read every page of the file and extract ALL visible text "
                        "(every heading, paragraph, bullet point, table cell — every word on every page). "
                        "Do NOT summarise or fabricate; copy the actual text verbatim.\n\n"
                        f"Then call: analyze_document_content("
                        f"content_sample=<full extracted text>, filename='{attached_name}')"
                    ),
                }
    ...
    def upload_document_to_knowledge_base(
        ...
        if not extracted_text and not source:
            ...
            if attached_name:
                return {
                    ...
                    "message": (
                        f"The file '{attached_name}' is in your multimodal context — you CAN read it.\n\n"
                        "ACTION REQUIRED: Read every page of the file and extract ALL visible text verbatim. "
                        "Then call upload_document_to_knowledge_base again with "
                        "extracted_text=<full text> and display_name='{attached_name}'."
                    ),
                }
            return {
                "status": "error",
        ...
        agent_dept_map: dict = cfg.config.get("agent_dept_map", {})
        ...
        if scope == "department":
            ...
            if not dept_list:
                # Auto-infer department from the requesting agent
                inferred = _dept_from_agent(source_agent, agent_dept_map)
                if inferred:
                    dept_list = [inferred]
                    logger.info(
                        "rag_ingest: auto-detected dept '%s' from source_agent '%s'",
                        inferred, source_agent,
                    )
        ...
        try:
            ...
            if extracted_text and extracted_text.strip():
                ...
                _register(rag_file_name, scope, dept_list, scope_registry_uri,
                          personal_owner=personal_owner, display_name=display_name,
                          user_id=user_id)
                logger.info(
            ...
            return _upload_no_metadata(
                ...
                personal_owner=personal_owner, user_id=user_id,
            )
...
def _register(
    ...
    resolved_name = display_name or file_name.split("/")[-1]
    if scope == "organization":
        register_org_file(file_name, scope_registry_uri)
    elif scope == "personal" and personal_owner:
        register_personal_file(personal_owner, file_name, resolved_name, scope_registry_uri)
    else:
        register_dept_files(file_name, dept_list, scope_registry_uri)
        # Track per-department which user uploaded this file
        if user_id:
            for dept in dept_list:
                register_dept_user_file(dept, user_id, file_name, resolved_name, scope_registry_uri)
