...
Registry JSON schema (stored in GCS):
{
  ...
  "dept_user_files": {
    "hr": {
      "user@company.com": [
        {"rag_file_name": "projects/.../ragFiles/qqq", "display_name": "policy.pdf"},
        ...
      ]
    },
    ...
...
# ---------------------------------------------------------------------------
# Per-department user file registry  (dept_user_files[dept][user_id] = [...])
# Tracks who uploaded which files within each department.
# Access to dept files is still governed by dept_files; this is for audit/listing.
# ---------------------------------------------------------------------------

def register_dept_user_file(
    department: str,
    user_id: str,
    rag_file_resource_name: str,
    display_name: str,
    ...
    """Record that user_id uploaded a file to a specific department."""
    if not department or not user_id or not rag_file_resource_name:
        return
    data = _registry_read(registry_uri)
    dept_user: dict = data.setdefault("dept_user_files", {})
    dept_section: dict = dept_user.setdefault(department.lower().strip(), {})
    user_files: list = dept_section.setdefault(user_id, [])
    entry = {"rag_file_name": rag_file_resource_name, "display_name": display_name}
    if not any(f.get("rag_file_name") == rag_file_resource_name for f in user_files):
        user_files.append(entry)
    _registry_write(data, registry_uri)
    logger.info(
        "scope_registry: dept-user-registered %s → dept=%s user=%s",
        rag_file_resource_name, department, user_id,
    )


def list_dept_user_files(department: str, user_id: str, registry_uri: str) -> list[dict]:
    """Return files uploaded by user_id to the given department."""
    data = _registry_read(registry_uri)
    return list(
        data.get("dept_user_files", {})
            .get(department.lower().strip(), {})
            .get(user_id, [])
    )


def remove_dept_user_file(
    department: str,
    user_id: str,
    rag_file_resource_name: str,
    registry_uri: str,
) -> None:
    """Remove a file from user_id's upload record for a department."""
    data = _registry_read(registry_uri)
    dept_section = data.get("dept_user_files", {}).get(department.lower().strip(), {})
    if user_id in dept_section:
        dept_section[user_id] = [
            f for f in dept_section[user_id]
            if isinstance(f, dict) and f.get("rag_file_name") != rag_file_resource_name
        ]
    _registry_write(data, registry_uri)
    logger.info(
        "scope_registry: dept-user-removed %s from dept=%s user=%s",
        rag_file_resource_name, department, user_id,
    )
...
def analyze_document(content: bytes, filename: str) -> dict[str, str]:
    ...
    try:
        ...
        # Escape any { } in user content before calling .format() to prevent
        # KeyError when document text contains curly braces (e.g. code, JSON,
        # salary ranges). After escaping, .format() converts {{ → { and }} → }.
        safe_filename = filename.replace("{", "{{").replace("}", "}}")
        safe_sample = sample.replace("{", "{{").replace("}", "}}")
        response = model.generate_content(
            _ANALYSIS_PROMPT.format(filename=safe_filename, sample=safe_sample),
